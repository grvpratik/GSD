...
export async function searchController(c: any) {
	...
	const validated = await ValidateIdeaController(value, c);
	if (!validated || !validated.isValid) {
		return c.json({
			status: "error",
			message: validated.reasoning ?? "enter valid prompt",
		});
	}
	try {
		const overviewResult = await OverviewController(value, c);

		const featureResult = await FeatureController(value, c);
		const marketResult = await MarketController(value, c);
		console.log(overviewResult, featureResult, marketResult);
	} catch (error) {}
	return c.text("fuck yeah");
}
